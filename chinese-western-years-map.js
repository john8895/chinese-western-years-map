//json: http://data.chinese-linguipedia.org/content/application/datapedia/api/opendata_api.php?method=getdata&format=json&dataid=50&start=0&amount=1000
...
//     // xhr.open('get', 'http://data.chinese-linguipedia.org/content/application/datapedia/api/opendata_api.php?method=getdata&format=json&dataid=50&start=0&amount=1000');
//     xhr.open('get', 'chinese-western.json');
//     xhr.send();
//
// //progress loading效果
...
function filterList() {
    ...
    for (let i = 0; i < trObj.length; i++) {
        ...
        //     trObj[i].setAttribute('data-year','123')
        // }else{
...
function changeLunar(thisItem) {
    ...
    // getLunarDay(listYear);
    // let jsonData = getLunarData();
    // console.log(jsonData.west_year.typeOf(listYear));
    console.log(jsonData);
    for (let i = 0; i < jsonData.length; i++) {
        let textValue = jsonData[i].west_year;
        if (textValue.indexOf(filterYear) > -1) {
...
function getLunarDay(listYear) {
    let uYear = parseInt(document.getElementById('u_year').value) || 0;
